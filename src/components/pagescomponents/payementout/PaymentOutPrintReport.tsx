...
interface PaymentOutPrintReportProps {
  records: any[];
...
export function PaymentOutPrintReport({
  records,
  selectedMonth,
  businessProfile,
}: PaymentOutPrintReportProps) {
  ...
  return (
    ...
    <div className="print-area bg-white text-black font-sans w-full max-w-[794px] mx-auto px-10 py-6">
      ...
      <div className="mb-4 space-y-1.5">
        ...
        <p className="text-sm font-bold">Transaction type: Payment-Out</p>
      ...
      <table className="w-full text-xs mb-4 border-collapse">
        <thead style={{ backgroundColor: '#D3D3D3', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
          <tr className="border border-gray-500 text-left">
            ...
            <th className="py-1.5 px-2 font-bold text-right">Paid</th>
          </tr>
        </thead>
        <tbody>
          {records.map((record, idx) => (
            <tr key={record.id || idx} className="border-b border-gray-300 text-xs">
              ...
              <td className="py-1 px-2 border-r border-gray-300">{record.paymentNo || record.payment_no || record.receiptNo || record.receipt_no || ""}</td>
              <td className="py-1 px-2 border-r border-gray-300">{record.partyName || record.party_name || ""}</td>
              <td className="py-1 px-2 border-r border-gray-300">Payment-Out</td>
