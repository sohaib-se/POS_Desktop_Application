...
export function AllTransactionsReport({ onBack, onEditInvoice }: AllTransactionsReportProps) {
  ...
  const loadTransactions = useCallback(async () => {
    try {
      ...
      if (salesRes && salesRes.ok) {
        ...
        allTx = allTx.concat(sales.map((s: any) => ({
          id: `sale-${s.id}`,
          originalId: s.id,
          date: s.date,
          type: "Sale" as const,
          ...
          monthKey: getMonthKeyFromDate(s.date),
          rawInvoice: {
            id: s.id,
            invoiceNo: s.invoice_no,
            date: s.date,
      ...
      if (purchasesRes && purchasesRes.ok) {
        ...
        allTx = allTx.concat(purchases.map((p: any) => ({
          id: `purchase-${p.id}`,
          originalId: p.id,
          date: p.date,
          ...
          monthKey: getMonthKeyFromDate(p.date),
          rawInvoice: {
            id: p.id,
            invoiceNo: p.invoice_no,
            date: p.date,
  ...
  const monthOptions = useMemo(() => {
    const uniqueMonths = new Set(transactions.map((row) => row.monthKey));
  ...
  const selectedMonthRows = useMemo(() => {
    ...
    return transactions.filter((row) => row.monthKey === selectedMonthKey);
      ...
      >
        <div className="flex-1 max-w-sm bg-[#F6F0FB] rounded-xl p-4 border border-[#E8D7F6]">
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm text-[#6B6B83]">Total Sales</span>
          </div>
        ...
        <div className="flex-1 max-w-sm bg-[#E6F4EA] rounded-xl p-4 border border-[#CEEAD6]">
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm text-[#3D7D50]">Total Purchase Bill</span>
          </div>
