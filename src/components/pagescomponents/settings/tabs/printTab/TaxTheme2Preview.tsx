...
export function TaxTheme2Preview({ color, sale }: { color?: string, sale?: BillPreviewSaleData }) {
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", overflowX: "hidden", width: "100%", display: "flex", justifyContent: "center" }}>
      <div className="print-scroll-area" style={{ background: "#fff", padding: 20, fontFamily: "Inter, system-ui, sans-serif", width: "100%", maxWidth: "800px", border: "1px solid #000" }}>
        <div style={{ textAlign: "center", fontWeight: 700, fontSize: companyNameSize, marginBottom: 10 }}><EditableText textKey="title" defaultText="Sales" /></div>
        ...
        <>
          ...
          {data.description && (
            <div style={{ marginBottom: 10, padding: "8px", border: `1px solid ${BORDER}` }}>
            ...
            <table style={{ width: "100%", fontSize: invoiceFontSize, borderCollapse: "collapse", border: `1px solid ${BORDER}`, marginBottom: 10 }}>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ padding: "2px 4px", borderRight: `1px solid ${BORDER}` }}>Rs {(data.subtotal / 2).toFixed(2)}</td>
                  <td style={{ padding: "2px 4px", borderRight: `1px solid ${BORDER}` }}>{(data.taxRate * 100 / 2).toFixed(1)}%</td>
                  <td style={{ padding: "2px 4px" }}>Rs {(data.taxAmount / 2).toFixed(2)}</td>
                </tr>
                <tr>
          ...
          <table style={{ width: "100%", fontSize: invoiceFontSize, borderCollapse: "collapse", border: `1px solid ${BORDER}` }}>
            <thead>
              <tr style={{ background: themeBg, color: "#fff" }}>
                ...
                <th style={{ padding: "4px 6px", textAlign: "left", width: "50%" }}>{data.termsAndConditions ? "Terms and conditions:" : ""}</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td style={{ padding: "8px", verticalAlign: "top" }}>
                  {data.termsAndConditions && <div style={{ color: TEXT_MUTED, marginBottom: 10 }}>{data.termsAndConditions}</div>}
                  <div style={{ textAlign: "right" }}>
