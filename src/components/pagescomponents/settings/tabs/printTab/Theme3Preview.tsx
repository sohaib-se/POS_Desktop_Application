...
export function Theme3Preview({ color, sale }: { color?: string, sale?: BillPreviewSaleData }) {
  ...
  const borderStyle = `1px solid ${themeBg}`; // Vertical borders color
  ...
  return (
    <div style={{ background: "#fff", padding: 30, fontFamily: "Inter, system-ui, sans-serif", border: "1px solid #e2e8f0" }}>
      ...
      {/* Table */}
      <table style={{ width: "100%", fontSize: invoiceFontSize, borderCollapse: "collapse", marginBottom: 24, borderLeft: borderStyle, borderRight: borderStyle }}>
        <thead>
          <tr style={{ borderTop: `2px solid ${themeBg}`, borderBottom: `2px solid ${themeBg}` }}>
            <th style={{ padding: "8px 4px", textAlign: "left", fontWeight: 700, color: TEXT_DARK, borderRight: borderStyle }}>#</th>
            <th style={{ padding: "8px 4px", textAlign: "left", fontWeight: 700, color: TEXT_DARK, borderRight: borderStyle }}>Item name</th>
            <th style={{ padding: "8px 4px", textAlign: "center", fontWeight: 700, color: TEXT_DARK, borderRight: borderStyle }}>Quantity</th>
            <th style={{ padding: "8px 4px", textAlign: "center", fontWeight: 700, color: TEXT_DARK, borderRight: borderStyle }}>Unit</th>
            <th style={{ padding: "8px 4px", textAlign: "right", fontWeight: 700, color: TEXT_DARK, borderRight: borderStyle }}>Price/ Unit</th>
            <th style={{ padding: "8px 4px", textAlign: "right", fontWeight: 700, color: TEXT_DARK }}>Amount</th>
          </tr>
        </thead>
        <tbody>
          {data.lineItems.map((item, idx) => (
            <tr key={item.id ?? idx} style={{ borderBottom: borderStyle }}>
              <td style={{ padding: "8px 4px", color: TEXT_DARK, borderRight: borderStyle }}>{idx + 1}</td>
              <td style={{ padding: "8px 4px", color: TEXT_DARK, borderRight: borderStyle }}>{item.name}</td>
              <td style={{ padding: "8px 4px", textAlign: "center", color: TEXT_DARK, borderRight: borderStyle }}>{item.quantity}</td>
              <td style={{ padding: "8px 4px", textAlign: "center", color: TEXT_DARK, borderRight: borderStyle }}>{item.unit !== "NONE" ? item.unit : ""}</td>
              <td style={{ padding: "8px 4px", textAlign: "right", color: TEXT_DARK, borderRight: borderStyle }}>Rs {item.price.toFixed(2)}</td>
              <td style={{ padding: "8px 4px", textAlign: "right", color: TEXT_DARK }}>Rs {item.amount.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr style={{ borderBottom: `2px solid ${themeBg}`, fontWeight: 700 }}>
            <td colSpan={2} style={{ padding: "8px 4px", color: TEXT_DARK, borderRight: borderStyle }}>Total</td>
            <td style={{ padding: "8px 4px", textAlign: "center", color: TEXT_DARK, borderRight: borderStyle }}>{data.lineItems.reduce((sum, i) => sum + i.quantity, 0)}</td>
            <td colSpan={2} style={{ padding: "8px 4px", borderRight: borderStyle }}></td>
