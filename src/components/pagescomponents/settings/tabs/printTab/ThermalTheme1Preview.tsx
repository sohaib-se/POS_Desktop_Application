...
export function ThermalTheme1Preview({ sale }: { sale?: BillPreviewSaleData }) {
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginTop: 4, paddingLeft: 40, fontSize: invoiceFontSize * 0.9 }}>
        {data.discountAmount > 0 && (
          <div style={{ display: "flex", width: "100%", justifyContent: "space-between" }}>
            <span style={{ flex: 1, textAlign: "right" }}><EditableText textKey="lbl_discount" defaultText="Disc." />({data.discountPercent}%)</span>
            <span style={{ width: 20, textAlign: "center" }}>:</span>
            <span style={{ width: 80, textAlign: "right" }}>-{data.discountAmount.toFixed(2)}</span>
          </div>
        )}
        {data.taxAmount > 0 && (
          <div style={{ display: "flex", width: "100%", justifyContent: "space-between" }}>
            ...
            <span style={{ width: 20, textAlign: "center" }}>:</span>
            <span style={{ width: 80, textAlign: "right" }}>{data.taxAmount.toFixed(2)}</span>
          </div>
        )}
        <div style={{ display: "flex", width: "100%", justifyContent: "space-between" }}>
