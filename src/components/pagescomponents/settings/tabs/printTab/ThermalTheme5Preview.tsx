...
export function ThermalTheme5Preview({ sale }: { sale?: BillPreviewSaleData }) {
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e2e8f0", padding: "16px", fontFamily: "Inter, system-ui, sans-serif", fontSize: invoiceFontSize, color: TEXT_DARK, width: "100%", margin: "0 auto", lineHeight: 1.4, fontWeight: thermalTextBold ? 600 : 400, overflowX: "hidden", wordBreak: "break-word" }}>
      ...
      <div style={dash} />
      <div style={{ textAlign: "center", fontWeight: 600 }}>{data.partyName}</div>
      <div style={dash} />
      ...
      <div style={{ display: "flex", fontWeight: 600, paddingLeft: 20 }}>
      ...
      {data.lineItems.map((item, idx) => (
        <div key={item.id ?? idx} style={{ marginTop: 4 }}>
          ...
          <div style={{ display: "flex", paddingLeft: 20 }}>
            <span style={{ flex: 1 }}>{item.quantity}{item.unit !== "NONE" ? item.unit : ""}</span>
      ...
      <div style={{ display: "flex", fontWeight: 600 }}>
        ...
        <span style={{ width: 80, textAlign: "center" }}>Items: {data.lineItems.length}</span>
