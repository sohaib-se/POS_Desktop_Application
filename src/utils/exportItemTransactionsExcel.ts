// Utility: export Item Transactions to a styled .xlsx file
...
export interface ItemTransactionExportRow {
  type: string;
  invoiceNo: string;
  partyName: string;
  date: string;
...
export function exportItemTransactionsToExcel(
  itemName: string,
  records: ItemTransactionExportRow[],
  ...
  const filename = `item_transactions_${safeName}_${monthLabel}_${yearLabel}.xlsx`;
  ...
  const dataRows = records.map((r) => [
    ...
    { v: String(r.partyName || ""),                           t: "s", s: dataStyleLeft  },
    { v: formatDate(r.date || ""),                            t: "s", s: dataStyleLeft  },
    { v: Number(r.quantity).toLocaleString(),                 t: "s", s: dataStyleRight },
    { v: String(r.unit || ""),                                t: "s", s: dataStyleLeft  },
    { v: `${currencyCode} ${Number(r.price).toFixed(2)}`,    t: "s", s: dataStyleRight },
    { v: String(r.status || ""),                              t: "s", s: dataStyleLeft  },
  ]);
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Item Transactions");
