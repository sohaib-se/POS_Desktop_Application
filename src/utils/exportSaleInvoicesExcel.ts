// Utility: export Sale Invoices to a styled .xlsx file
...
export interface SaleInvoiceExportRow {
  date: string;
...
export function exportSaleInvoicesToExcel(
  rows: SaleInvoiceExportRow[],
  monthKey: string,       // e.g. "Sep-2026"
  currencyCode: string
) {
  // ── Filename ────────────────────────────────────────────────────────────────
  const safeMonth = (monthKey || "all").replace(/\s+/g, "_");
  const filename = `sale_invoices_${safeMonth}.xlsx`;
